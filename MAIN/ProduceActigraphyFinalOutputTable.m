function ProduceActigraphyFinalOutputTable(FoundVideoFilesPath,BBParentPath,GeneralOutputsPath,BBlist,BBToAnalyze)
...
output_found_videos_file_name = fullfile(FoundVideoFilesPath,'FoundVideoFiles.mat');
actigraphy_output_parent_path_root = fullfile(BBParentPath, 'ActigraphyResults\BB');
...
for i=1:num_boxes_to_process
    ...
    if current_included_bbIDs(i)
        ...
        curr_activity_timetable_hourly_binned_counts = retime(curr_activity_timetable,'minutely','count'); % DAVID'S CHANGES: TURNED TO MINUTELY
        video_frames_per_bin = curr_activity_timetable_hourly_binned_counts.NumChangedPixels;
        
        
        curr_activity_timetable_hourly_binned_mean = retime(curr_activity_timetable,'minutely','mean'); % DAVID'S CHANGES HERE TOO SAME DEAL AS ABOVE
        curr_activity_timetable_hourly_binned_sum = retime(curr_activity_timetable,'minutely','sum');% AND HERE
