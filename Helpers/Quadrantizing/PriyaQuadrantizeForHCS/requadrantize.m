...
for k=1:num_boxes
   %if BBToAnalyze(k) == 1 4.2023 Josh-TODO only quadrantize boxes we're interested in.
    ...
    for i = 1:num_boxes
	    ...
	    for j = 1:6      %for j = 1:total_vids 1-6 for quick testing
            ...
            videoHeight = str2num(videoMetadata(8:10));
            videoWidth = str2num(videoMetadata(12:14));
            videoNumFrames = str2num(videoMetadata(16:end));
	...
	frames=zeros(1,x); %refer to all_videos_output_data{1, i}.videoFilesData.numFrames    for the name of BB01videos. 
